...
import { user,
    ...
    validLoginBody,
    userRegistered } from './mocks/users.mock';
...
describe('Login tests', () => {
    ...
    it('should not can do the login with invalid body', async function() {
      ...
      it('should not can access wih userWithInvalidEmail', async function() {
        const { status, body } = await chai.request(app).post('/login').send(userWithInvalidEmail);
      ...
      it('should not can access with userWithInvalidPassword ', async function() {
        const { status, body } = await chai.request(app).post('/login').send(userWithInvalidPassword);
